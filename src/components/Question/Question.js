...
import { Col, Container, Row, Alert, FormGroup, CustomInput, Spinner } from 'reactstrap'
                    ...
                    <Form onSubmit={handleSubmit}>
                        ...
                        {questions.map((question, index) =>
                            <React.Fragment key={index}>
                                ...
                                <span>
                                    <FormGroup onChange={(e) => updatingQuestion(e, index)} key={index}>
                                        <CustomInput type="radio" name={`question${question}`} id={`${index}Sim`} label="Sim" value="S"></CustomInput>
                                        <CustomInput type="radio" name={`question${question}`} id={`${index}Não`} label="Não" value="N"></CustomInput>
                                        <CustomInput type="radio" name={`question${question}`} id={`${index}NS`} label="Não sei" value="NS"></CustomInput>
                                    </FormGroup>
                                </span>
                            </React.Fragment>
                        )}
                        <ButtonStyled className="px-5 py-2 mY-3">Salvar</ButtonStyled>
                        {messageSuccess && (<Alert className="mt-3" color="success">{messageSuccess}</Alert>)}
                        {messageError && (<Alert className="mt-3" color="danger">{messageError}</Alert>)}
                    </Form>
                </Col>
            </Row>
