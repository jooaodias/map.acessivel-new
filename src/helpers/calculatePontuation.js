function calculatePontuation(question) {
  ...
  question?.map(quest => {
    // console.log(quest)
    return quest[0].map((item, index) => {
      // console.log(item.value, index)
      if (index < 4) {
        return (contBlind += verifyPontuation(item.value, 1));
      } else if (index < 8) {
        return (contWheilchair += verifyPontuation(item.value, 2));
      } else {
        return (contDeaf += verifyPontuation(item.value, 3));
      }
    });
  });
